...
#pragma endregion
		...
		string generate() {
			string name;
			do {
				name = this->mygenerate + to_string(randomize());
...
#pragma region Internal Calcutions
				...
				myenv[codexec2[0]] = intValue(randomize());
			}
		...
		if (codexec.size() >= 2) {
			...
			if (codexec.size() >= 2) {
				if (codexec[1] == "time_set") {
					auto tm = time(NULL);
					auto lt = *localtime(&tm);
					myenv["this.year"] = intValue(lt.tm_year);
					myenv["this.month"] = intValue(lt.tm_mon);
					myenv["this.day"] = intValue(lt.tm_mday);
					myenv["this.hour"] = intValue(lt.tm_hour);
					myenv["this.minute"] = intValue(lt.tm_min);
					myenv["this.second"] = intValue(lt.tm_sec);
					myenv["this.ms"] = intValue(0);						// Unsupported
					myenv["this.week"] = intValue(lt.tm_wday);	// Not sure
				}
...
#pragma region Preset calls
	...
	intcalls["set_color"] = [](string args, varmap &env) -> intValue {
		setColor(int(calculate(args, env).numeric));
		return null;
	};
...
#pragma endregion
	...
	if (argc <= 1 && !file.length() && !code.length()) {
		cout << "\033[93mUsage: " << argv[0] << " filename [options]\033[0m" << endl;
		return 1;
	}
...
#pragma region Read Options
	...
	if (file == "--version") {
		// = true;
		cout << "\033[94m" << version_info << "\033[0m" << endl;
		return 0;
	}
...
#pragma endregion
	...
	int rescode = preRun(code, reqs, callers).numeric;
	printf("\033[0m"); // Clean up environment
	return rescode;
}
